...
# monorepo and the server imports `@anime/shared` as a workspace dependency;
# a build context of `server/` alone has no `shared/` to resolve it against and
# no root lockfile to install from.
#
#   docker build -t speed-anime-evolve-server .
#   docker run -e PORT=2591 -p 2591:2591 speed-anime-evolve-server
...
# imports require: `@anime/shared` resolves to `shared/dist/index.js`.
RUN npm run build:server

# Drop to production dependencies in place. This keeps the workspace symlinks
# that `@anime/shared` resolves through - deleting node_modules and
...
# hosts, injects one - and `serverConfig` falls back to 2591.
ENV HOST=0.0.0.0
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=15s --retries=3 \
  CMD node -e "fetch('http://127.0.0.1:'+(process.env.PORT||2591)+'/health').then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"
