...
const main = async (): Promise<void> => {
  ...
  setBootStatus('Loading the course…');
  game.loadingStep('Loading the course…');
  await paint();
  ...
  if (clientConfig.debug) {
    ...
    (window as Window & { __anime?: DebugHandle }).__anime = { game, loop };
  }
...
/** Shape of the dev-only `window.__anime` handle. */
