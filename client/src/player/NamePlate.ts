...
import { visibleName } from '@anime/shared';
import { DEFAULT_PORTRAIT, drawPortrait, portraitFor } from '../bloxity/Portraits.js';
 ...
 * CLEARANCE - HEIGHT / 2: a clear gap of air over hats and spiky hair.
 */
...
export class NamePlate {
  ...
  /**
   * @param size how big to draw the plate: 1 over other players; smaller over
   *             your OWN runner, so it never covers the course ahead of the
   *             chase camera.
   */
  constructor(private readonly size = 1) {
    ...
    this.sprite.scale.set(MAX_WIDTH * size, HEIGHT * size, 1);
   ...
   * @param height      the runner's own height, so the plate clears every character
   */
  set(displayName: string, avatarUrl: string, height: number): void {
    this.name = visibleName(displayName);
    // Every player has a face: their portal portrait, else the portal's default.
    this.face = avatarUrl || DEFAULT_PORTRAIT;
    this.repaint();
    // Same gap of air over the head whatever the plate's size.
    this.sprite.position.set(0, height + CLEARANCE - (HEIGHT * (1 - this.size)) / 2, 0);
  ...
  private paint(image: HTMLImageElement | null): void {
     ...
     * over the runner whatever anybody is called.
     *
     * The face is ALWAYS there: until (or unless) the portrait loads, a
     * bright badge with the player's initial stands in for it.
     */
    const faceSize = height * FACE_SHARE;
    const textBand = height - faceSize;
    let size = textBand * 0.74;

    if (image) drawPortrait(ctx, image, (width - faceSize) / 2, faceSize / 2, faceSize);
    else this.badge(ctx, (width - faceSize) / 2, faceSize / 2, faceSize);
  ...
  /** The stand-in face: a bright circle with the player's initial. */
  private badge(ctx: CanvasRenderingContext2D, x: number, centreY: number, size: number): void {
    const colors = ['#ff4fa8', '#2f7dff', '#19c2a0', '#8a4dff', '#ff8a1f', '#2ed8ff'];
    let hash = 0;
    for (const ch of this.name) hash = (hash * 31 + ch.charCodeAt(0)) >>> 0;
    const r = size / 2;
    ctx.beginPath();
    ctx.arc(x + r, centreY, r - 2, 0, Math.PI * 2);
    ctx.fillStyle = colors[hash % colors.length]!;
    ctx.fill();
    ctx.lineWidth = 3;
    ctx.strokeStyle = '#ffffff';
    ctx.stroke();
    ctx.font = `900 ${size * 0.55}px "Fredoka", "Baloo 2", "Nunito", "Segoe UI", system-ui, sans-serif`;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillStyle = '#ffffff';
    ctx.fillText([...this.name][0]?.toUpperCase() ?? '?', x + r, centreY + size * 0.03);
  }
