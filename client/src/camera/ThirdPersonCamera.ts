import { CAMERA } from '@anime/shared';
...
export class ThirdPersonCamera {
  ...
  update(delta: number, speed: number): void {
    ...
    if (!this.initialised) {
      ...
      // Tighter the faster the runner goes: a fixed rate lags a sprinter by metres.
      this.followed.lerp(this.target, 1 - Math.exp(-(CAMERA.followLerp + speed * 0.6) * delta));
    }
