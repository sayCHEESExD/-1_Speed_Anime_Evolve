 ...
 * runner footstep, and those four are used as they are - a tune, a real impact
...
const LAYERED: ReadonlySet<SoundName> = new Set<SoundName>([]);
...
export type SoundName =
  ...
  | 'rebirth'
  | 'claim'
  /** A character, trail or charm bought. */
  | 'unlock'
  /** Something equipped. */
  | 'buy'
...
export class AudioManager {
   ...
   *
   * The supplied `walk.mp3` is nearly three seconds of a runner WALKING -
   * several footfalls, not one - so firing it on every stride would restart it
   * before it had played its first step and the runner would sound like it was
  ...
  resume(): void {
    ...
    if (!this.context) {
       ...
       * so low that the runner could not be heard walking. This catches the
   ...
   *
   * @param active true while the runner is on the ground and actually moving
  ...
  setFootsteps(active: boolean, pace: number): boolean {
     ...
     *
     * The same reasoning as the animation's cadence clamp: a late-game runner
    ...
    // than this runner's, and the gait it has to agree with is a slow one.
     ...
     * them - and the music reaches the master at MUSIC_GAIN, 0.33. A walk that
     ...
     *
     * The band is narrow on purpose. A runner walking slowly is still a runner
  ...
  play(name: SoundName, intensity = 1, delay = 0): void {
    ...
    switch (name) {
      case 'jump':
        ...
        break;
      case 'whoosh':
        // The supplied shine, falling back to the synthesised rush.
        if (this.playSample('whoosh', now, 0.55 * Math.max(0.6, level))) break;
        this.blip(now, 'sawtooth', 900, 120, 0.35, 0.12 * level);
        this.blip(now, 'sine', 400, 1200, 0.25, 0.1 * level);
        break;
      case 'refuse':
      ...
      case 'step':
         ...
         *
         * A short low thud, and LOW is the point: this is a runner's foot hitting the ground.
         */
