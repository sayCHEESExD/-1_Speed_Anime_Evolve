...
export class LocomotionCycle {
  ...
  advance(delta: number, speed: number, _multiplier: number, sprinting: boolean): number {
    const frequency = clamp(
      speed / (LOCOMOTION.strideDistance + speed * LOCOMOTION.strideGrowth),
      LOCOMOTION.minFrequency,
      LOCOMOTION.maxFrequency,
    );
    this.phase = (this.phase + frequency * TAU * delta) % TAU;

    // The sprint is the simulation's, not the speed's: a level-40 runner jogging
    // is still jogging. It eases in, so the arms sweep back rather than snap.
    const target = sprinting ? 1 : 0;
    this.sprintWeight += (target - this.sprintWeight) * (1 - Math.exp(-5 * delta));
    return frequency;
  }
