...
const DEFAULT_PFP = DEFAULT_PORTRAIT;

/**
 * The portal's face inside the game.
 *
 * An account chip on the right of the screen, and the panels behind it:
 * friends and cosmetics. It owns no state - the chip is redrawn from
 * whatever `Bloxity.onUserChanged` last said, and every figure it shows is
 * fetched when the panel opens rather than cached.
 *
 * The panels extend the game's own `Panel`, which is what keeps movement
 * suppressed while one is open.
 */
export class BloxityPanel {
  ...
  private friendRow(friend: LegionFriend): HTMLDivElement {
     ...
     * over a runner, on a board and here - and never an id of any kind.
     */
